...
- `_persist_nickname` and `_share_id_for_session` are the shared halves, so Save,
  Save & Share and the `share-results-button` cannot drift apart. Save & Share builds the
  record from the **updated** session, so the card carries the name just typed.
...
- **Source of truth is the ranking CSVs**, not any `dcc.Store`: `data/input/prediction_ranking.csv` (cumulative "ALL", `mode="latest"`) plus `data/input/prediction_ranking_{A,B,C}.csv` (per data source, `mode="best"`), read per request by `_leaderboard_snapshot`. A visitor with no session sees the board; `user-info-store`'s `study_id` (a `State` in `display_page`) only adds the highlighted "You" row and the "your place" hero. Those CSVs are **gitignored** and header-only in a fresh checkout, so the page legitimately renders its "no ranked games yet" empty state locally — seed synthetic rows if you need to look at a populated board. The landing "games played so far" counter reads **all** starters from `prediction_statistics.csv`, then "out of which N completed the task" (unique people with at least `MIN_USEFUL_ROUNDS` in any category).
- **`create_highscore_page` shares its building blocks with `/final`** — `_leaderboard_hero_children` and `_leaderboard_board` are used by both, so a change to row/hero markup shows up on both pages. Players who picked a nickname are shown by it; the rest stay anonymous (`Player N`). Never render `study_id` or `email_key`.
...
- **Arcade placement: one slot per finished game, among completers.** `_ranking_entries` (behind `_leaderboard_snapshot` / `_rank_from_ranking_csv`) does no grouping at all — every rankable row is its own board slot, so beating your own score leaves the old one standing below it, and one player can hold several slots. Ties break on the earlier timestamp (whoever got there first sits higher). Each slot keeps the nickname stored **on its own row** — the name that score was set under, not the player's current name. Do not "tidy" this by deduping; hiding a player's earlier *ranked* runs is the confusing behaviour this replaced. **Display filter:** runs with fewer than `MIN_USEFUL_ROUNDS` are excluded from the board and show as unranked on `/final` and the share card/image. Rank among completers is still lowest MAE; the floor is what stops a 1-round lucky score beating a full run.
  - `total` = slots, `rank` = the player's **best** slot, `players` = distinct people (only used for the stat chip, since `total` would just restate `games_played`). The per-source card subtitle uses `ui.highscore.scores_count`, not `players_count`.
  - The `Rounds` column (`ui.final.col_rounds`) is load-bearing: the overall CSV's rows are *cumulative*, so one player's two slots can read 12 rounds / MAE 20 and 24 rounds / MAE 14. Without the round count those look like an unexplained duplicate. `grid-template-columns` in `lang.css` and `mobile.css` must stay in sync with the four cells.
...
- **`_ranking_identities` / `_match_identity` are the per-*player* rollup and are deliberately NOT wired into any page.** They are kept for the planned individual stats page ("your best across every device"), and `tests/test_ranking_identity.py` exercises them so they cannot rot. That is where `mode` still means something (`latest` for the cumulative CSV, `best` per format) and where `identity_key` merging happens. Don't delete them as dead code, and don't reintroduce them into the boards.
