...
- **Screenshot harness (`uv run python scripts/mobile_shots.py`):** emulate with CDP `mobile:false` for device-width pages (deterministic — `mobile:true` triggers the flaky expand-to-fit), but `mobile:true` in landscape for `/prediction` (so the 1280 meta is honoured/scaled). Do NOT use `captureBeyondViewport:true` (it re-lays-out at a 1280 fallback) — instead grow the viewport height to `scrollHeight` and take a normal viewport capture; skip the height-grow for landscape and `/prediction` (their content is viewport-sized).
- **Landscape `/prediction` crops the chart bottom unless you re-apply the device metrics after the `1280` meta switch.** The meta swap is clientside and fires after first paint; under a CDP override Chromium does NOT re-fit the page scale, so the capture shows the unscaled top-left 740px slice and the x-axis + Submit fall off. Fix: `Emulation.clearDeviceMetricsOverride` then re-apply the **same** `mobile:true` 740×360 metrics (forces a re-emulation that re-reads the now-1280 meta and scales the layout to fit). **Do NOT instead set `screenWidth:1280`/`mobile:false`** — that makes *device-width* 1280, failing the `max-device-width:1024` gate on the immersive landscape CSS, so the navbar + instructions reappear and the chart drops below the fold. Device-width must stay ≤1024; only the *layout* viewport is 1280.
...
- **localStorage is device-local — no cross-device resume.** Every session store is `STORAGE_TYPE=local`, so a game started on a phone cannot continue on a desktop (or vice-versa). Only the read-only on-disk share record crosses devices. This is the architecture, not a bug; a server-side resume code is the designed fix (backlog).
