...
_CONSENT_NOTICE_TRANSLATIONS: Final[dict[str, list[str]]] = {
    ...
    "bg": [
        ...
        "Прогнозиране на глюкозни траектории въз основа на предходни модели на стойности: онлайн сравнително изследване",
        "Кратко заглавие: изследване за прогнозиране на глюкоза Sugar-Sugar",
        "Моля, прочетете внимателно следния текст, преди да вземете решение. Ако имате въпроси, обърнете се към ръководството на изследването.",
        "Уважаеми участнико,",
        "бихме искали да ви попитаме дали желаете да участвате в научно изследване. Целта на това изследване е за първи път систематично да се проучи колко точно хората могат да прогнозират краткосрочни глюкозни траектории въз основа на CGM данни (данни от устройство за непрекъснато мониториране на глюкозата). Досега такъв сравнителен еталон не съществува. Вашето участие помага да се установи референтна стойност за човешката точност на прогнозиране, която в дългосрочен план може да се използва и за оценка на компютърни модели за прогнозиране. Такива модели за прогнозиране се използват все по-често в изследванията на диабета и в грижата за хора с диабет и могат да бъдат валидирани и сравнявани чрез събраните тук данни.",
        "Изследването се провежда в IBIMA (Институт по биостатистика и информатика в медицината и изследванията на стареенето) към Университетска медицина Росток, в сътрудничество с HEALES (Healthy Life Extension Society, Брюксел), която е финансираща организация на изследването. Ръководители на изследването са Anton Kulaga (Университетска медицина Росток) и Livia Zaharia (HEALES). Планирани са най-малко 200 участници.",
        "Участието в изследването няма влияние върху каквото и да е медицинско лечение. Това е изцяло онлайн изследване без присъствени срещи и без медицински процедури.",
        "Как протича изследването?",
        "Участието се осъществява изцяло онлайн чрез уебприложението Sugar-Sugar и отнема около 15–20 минути.",
        "Първо ще попълните кратък регистрационен въпросник (възраст, пол, държава на пребиваване, статус на диабет, история на използване на CGM; по желание: тегло и ръст). След това ще изпълните от 6 до 12 задачи за прогнозиране. Всяка задача показва три часа исторически CGM данни от публично известен, анонимизиран набор от данни. Ще ви бъдат показани по-ранните времеви точки от времеви редове, събрани в други изследвания; от вас се очаква да предположите по-късните времеви точки. Ще начертаете очакваната глюкозна траектория за следващите 60 минути, като поставите 12 точки на графика. Преди оценяваните задачи ще ви бъдат предложени два незадължителни тренировъчни рунда.",
        "След всяка задача се показват действителните стойности на глюкозата и се изчисляват показатели за точност. В края ще получите обобщение на точността на прогнозирането си (определена въз основа на историческите данни), както и процентилен ранг.",
        "Точността на прогнозиране се сравнява в четири групи: хора с диабет и използване на CGM, хора с диабет без използване на CGM, хора без диабет с използване на CGM и хора без диабет без опит с CGM; въз основа на данните, събрани в началото, ще бъдете причислени към една от тези групи.",
        "Незадължително качване на собствени CGM данни",
        "Ако самите вие използвате CGM устройство, можете по желание да качите експортен файл (CSV или JSON). От него се извличат кратки сегменти, които се използват за допълнителни задачи за прогнозиране. Това позволява да се сравни дали прогнозирате собствените си данни по-точно от историческите данни. Дата и час не се показват в потребителския интерфейс. Каченият суров файл се изтрива в рамките на 30 дни; в псевдонимизиран вид се запазват само извлечените сегменти.",
        "Доброволно участие",
        "Участието в това изследване е доброволно. Ще бъдете включени само ако заявите съгласието си. Ако не участвате в изследването или по-късно пожелаете да се оттеглите от него, няма да претърпите никакви неблагоприятни последици. Можете да прекратите участието си по всяко време, без да посочвате причини. Частични данни от завършени рундове се включват само ако са завършени най-малко 6 валидни рунда.",
        "Кой може да участва?",
        "Могат да участват пълнолетни лица на възраст 18 и повече години с достъп до интернет. Допустими са както хора с диабет, така и хора без диабет.",
        "Какви възможни рискове, неудобства или странични ефекти са свързани с участието ви?",
        "Тъй като нашето изследване само събира данни, с участието не са свързани медицински рискове. Стойностите за точност имат значение изключително за научни цели и нямат диагностичен или клиничен смисъл.",
        "Какви възможни ползи произтичат от участието ви в изследването?",
        "Участието не ви носи лична полза за здравето. Участието допринася за систематичен сравнителен еталон на човешката способност за прогнозиране на стойности на глюкозата. Ще получите лично обобщение на точността, както и карта с резултати. Не се предвижда парично възнаграждение.",
        "Какви права и условия са свързани с участието?",
        "Можете да прекратите участието си в изследването по всяко време, дори без да посочвате причини. Ако не желаете да участвате или желаете да се оттеглите, няма да претърпите никакви неблагоприятни последици.",
        "Моля, имайте предвид, че за този проект и съпътстващото изследване не е сключена отделна застраховка, тъй като става дума за изцяло онлайн изследване без медицински процедури и следователно с минимален риск.",
        "Изследването е получило положително становище от компетентната етична комисия.",
        "Защита на данните",
        "По-долу бихме искали да ви информираме изчерпателно какво се случва с личните ви данни в рамките на изследването и как се отнасяме към тях.",
        "1. Кой е отговорен за обработването и съхранението на данните?",
        "Университетска медицина Росток е отговорна за обработването и съхранението на данните в рамките на изследването. HEALES оперира приложението Sugar-Sugar като обработващ в рамките на споразумение за обработване на данни (DPA) с Университетска медицина Росток. Можете да се свържете с отговорната институция на следните координати:",
        "Отговорна институция: Университетска медицина Росток – правоспособна подорганизация на Университета Росток",
        "Schillingallee 35, 18057 Rostock",
        "www.med.uni-rostock.de",
        "2. Към кого можете да се обърнете с въпроси относно изследването или защитата на данните?",
        "За да защитим възможно най-добре интересите ви, можете по всяко време да се обърнете и към следните лица с всякакви въпроси относно участието ви в изследването или общия ход на процедурата:",
        "Контакти на изследването:",
        "• Anton Kulaga: anton.kulaga@uni-rostock.de",
        "• Livia Zaharia: livia.zaharia@uni-rostock.de",
        "По въпроси за защита на данните е на разположение и длъжностното лице по защита на данните на Университетска медицина Росток:",
        "Doberaner Str. 142, 18057 Rostock",
        "0381 494 5155 | datenschutz@med.uni-rostock.de",
        "Ако желаете да упражните правото си да подадете жалба срещу неправомерно обработване на данни, моля, свържете се с компетентния надзорен орган:",
        "Държавен комисар за защита на данните и свободата на информацията M-V",
        "Schloss Schwerin, Lennéstr. 1, 19053 Schwerin",
        "info@datenschutz-mv.de",
        "3. Кои данни са ни необходими от вас за провеждане на изследването?",
        "В рамките на изследването се събират следните данни:",
        "Данни за връзка: електронен адрес (за регистрация, обработване на оттегляне и незадължително повторно уведомяване).",
        "Демографски данни: възраст, пол, държава на пребиваване; по желание: тегло и ръст.",
        "Данни, свързани със здравето: статус на диабет (да/не, тип, продължителност), история на използване на CGM.",
        "Данни от изследването: прогнозирани и, ако е приложимо, действителни стойности на глюкозата, показатели за точност (MAE, MSE, RMSE, MAPE).",
        "Предпочитания относно съгласието: документиране на решенията ви за съгласие.",
        "По желание – собствени CGM данни: при качване се запазват само извлечените сегменти в псевдонимизиран вид.",
        "4. Откъде получаваме тези данни?",
        "Всички данни се събират изключително пряко от вас като участник, чрез вашите въвеждания в уебприложението Sugar-Sugar (регистрационен въпросник, задачи за прогнозиране, незадължително качване на CGM). Данни от други източници не се получават. Правно основание за обработването е вашето изрично съгласие съгласно Art. 6(1)(a) GDPR и Art. 9(2)(a) GDPR.",
        "5. За какви цели са ни необходими данните ви и как са защитени?",
        "Изброените по-горе данни са необходими изключително за научното изследване на човешката способност за прогнозиране на глюкоза и се използват само за тази цел.",
        "Всяко участващо лице получава случаен идентификатор на изследването при регистрация. Всички изследователски данни са свързани изключително с този идентификатор. Отделен криптиран файл за съпоставяне, който свързва идентификатора на изследването с електронния адрес, се съхранява отделно и е достъпен само за ръководството на изследването (Anton Kulaga и Livia Zaharia). Този файл се използва изключително за обработване на искания за оттегляне и за незадължителни уведомления за резултати. За останалите членове на екипа на изследването следователно не е възможно да ви идентифицират.",
        "Данните се криптират при предаване и в покой. Съхранението се извършва на сървъри в рамките на Европейския съюз (хостинг, съответстващ на GDPR). Достъпът се регулира въз основа на роли.",
        "6. Къде и за колко време се съхраняват данните?",
        "Събраните в рамките на изследването данни първо се псевдонимизират. Очаква се файлът за съпоставяне (електронен адрес – идентификатор на изследването) да бъде унищожен 12 месеца след приключване на изследването. След това данните от изследването се анонимизират, тоест вече не позволяват никакъв извод за вашата самоличност.",
        "Псевдонимизираните изследователски данни се съхраняват 10 години в съответствие с германските изследователски стандарти. Качените сурови CGM файлове се изтриват в рамките на 30 дни.",
        "7. Кой ще научи за вашите данни? Ще бъдат ли данните ви споделяни или публикувани?",
        "Псевдонимизираните изследователски данни са достъпни само за екипа на изследването в IBIMA. Файлът за съпоставяне е достъпен само за ръководството на изследването (Anton Kulaga и Livia Zaharia). Резултатите от изследването се публикуват само в обобщен, анонимизиран вид. От публикациите не е възможно да се направи извод за отделни лица.",
        "8. Какви права във връзка със защитата на данните имате при участие в изследването?",
        "В съответствие с приложимите разпоредби за защита на данните (GDPR) имате право на достъп до съхраняваните данни, на коригиране на неточни данни, на изтриване на данни, на ограничаване на обработването, на възражение срещу необосновано обработване, както и право на преносимост на съхраняваните за вас данни. Имате също право да оттеглите съгласието си. Можете да упражните тези права по всяко време пред отговорната институция.",
        "Право на оттегляне",
        "По принцип можете да оттеглите съгласието си по всяко време, без да посочвате причини. Моля, свържете се с ръководството на изследването (anton.kulaga@uni-rostock.de). Оттеглянето ви влиза в сила от момента на подаването му. Обработването на данните ви до този момент остава законосъобразно. При оттегляне ще бъде започнато изтриване на личните данни. Данни от изследването, които вече са анонимизирани и следователно вече не могат да бъдат свързани с вас, ще продължат да се използват в рамките на изследването. Очаква се файлът за съпоставяне да бъде унищожен 12 месеца след приключване на изследването; след това индивидуално съпоставяне и следователно изтриване вече не са възможни.",
        "Правно основание: Art. 6(1)(a) GDPR и Art. 9(2)(a) GDPR.",
        "Декларация за съгласие за участие в изследването за прогнозиране на глюкоза Sugar-Sugar и свързаното с него обработване на данни",
        "Идентификатор на изследването на участващото лице: _________________________________ (присвоява се при регистрация)",
        "Следните съгласия се записват електронно в момента на регистрацията чрез отмятане на съответните полета.",
        "Участвам в изследването доброволно. Внимателно прочетох информацията за участника в изследването, включеното уведомление за защита на данните и свързаните с тях права.",
        "Информиран/а съм, че в рамките на изследването ще се събират, обработват и използват лични данни, по-специално моите здравни данни (статус на диабет, история на използване на CGM, резултати от прогнозирането).",
        "Както е посочено в информацията за участника, обработването се извършва в псевдонимизиран вид от екипа на изследването в IBIMA, Университетска медицина Росток. HEALES действа като обработващ по силата на DPA.",
        "Наясно съм, че ако данните ми се съхраняват анонимно, изтриване по мое искане вече не е възможно.",
        "Право на оттегляне",
        "Информиран/а съм, че мога да прекратя участието си по всяко време, без да посочвам причини, без никакви неблагоприятни последици за мен. Наясно съм също, че мога да оттегля съгласието си за обработване на данни по всяко време с действие за бъдещето, без това да засяга законосъобразността на обработването, извършено въз основа на съгласието до оттеглянето. Запознах се с информацията за оттегляне на съгласието в съответната информация за участника.",
        "С настоящото заявявам доброволното си съгласие да участвам в изследването за прогнозиране на глюкоза Sugar-Sugar. Разбирам, че мога да прекратя участието си по всяко време, без да посочвам причини, без никакви неблагоприятни последици за мен.",
        "С настоящото давам съгласието си, включително съответната информация за участника, за обработване на личните ми и здравни данни за следните цели:",
        "☐  Събиране, обработване и използване на данните ми, изброени в информацията за участника, по-специално на здравните ми данни (статус на диабет, история на използване на CGM, резултати от прогнозирането), в псевдонимизиран вид за провеждане на изследването от екипа на изследването в IBIMA, Университетска медицина Росток, както и от HEALES като обработващ. Съхранението се извършва на сървъри в рамките на Европейския съюз. Изследователските данни се съхраняват до 10 години след анонимизиране на файла за съпоставяне. Оттегляне е възможно до 12 месеца след приключване на изследването; след това идентификация вече не е възможна. Правно основание: Art. 6(1)(a) GDPR и Art. 9(2)(a) GDPR.",
        "☐  Участващото лице потвърждава, че е на 18 или повече години.",
        "Незадължителни съгласия (отказът няма влияние върху участието в изследването):",
        "☐  Съгласявам се да кача собствените си CGM данни за незадължителната задача за прогнозиране със собствени данни. Суровите данни ще бъдат изтрити в рамките на 30 дни; ще се запазят само псевдонимизираните извлечени сегменти.",
        "☐  Съгласявам се екипът на изследването да ме информира чрез регистрирания електронен адрес за резултатите от изследването след публикуването им.",
        "Уведомления от HEALES или свързани проекти ще бъдат представени на отделна страница след края на изследването и са напълно независими от участието в изследването и от съгласието.",
        "Това изследване е получило разрешение от Етичната комисия на Университетска медицина Росток. Регистрационен номер: Ref. number A 2026-0064. Получено от Етичната комисия: 27 февруари 2026.",
        "Въпроси или искания за оттегляне: anton.kulaga@uni-rostock.de | livia.zaharia@uni-rostock.de",
    ],
    "ja": [
        "参加者情報",
        "過去の値のパターンに基づくグルコース軌跡の予測：オンライン・ベンチマーク研究",
        "略称：Sugar-Sugar グルコース予測研究",
        "ご判断の前に、以下の文章をよくお読みください。ご質問がある場合は、研究責任者までご連絡ください。",
        "参加者の皆様へ、",
        "科学的研究へのご参加をお願いしたく、ご案内いたします。本研究は、CGMデータ（持続血糖モニタリング装置から得られるデータ）に基づいて、人が短期のグルコース軌跡をどの程度正確に予測できるかを、初めて体系的に調べることを目的としています。現時点では、そのようなベンチマークは存在しません。ご参加いただくことで、人による予測精度の参照値が確立され、長期的にはコンピュータによる予測モデルの評価にも用いることができます。こうした予測モデルは糖尿病研究および糖尿病ケアにおいてますます用いられており、ここで収集されるデータによって妥当性を検証し、比較することができます。",
        "本研究は、ロストック大学医学部のIBIMA（医学・加齢研究における生物統計学および情報学研究所）において、研究の資金提供者であるHEALES（Healthy Life Extension Society、ブリュッセル）との協力のもとで実施されます。研究責任者は Anton Kulaga（ロストック大学医学部）および Livia Zaharia（HEALES）です。少なくとも200名の参加者を予定しています。",
        "本研究への参加は、いかなる医学的治療にも影響を与えません。対面の予約や医学的処置を伴わない、純粋なオンライン研究です。",
        "研究はどのように進みますか。",
        "参加はすべてオンラインで、Sugar-Sugar ウェブアプリケーションを通じて行われ、所要時間はおよそ15～20分です。",
        "まず、短い登録用質問票（年齢、性別、居住国、糖尿病の状態、CGMの使用歴。任意：体重および身長）にご記入いただきます。その後、6から12件の予測課題を行っていただきます。各課題では、公開されている匿名化データセットから取り出した、過去3時間分のCGMデータが表示されます。他の研究で収集された時系列のうち、より早い時点が示されますので、その後の時点を推測していただきます。グラフ上に12点を置くことで、続く60分間の予想グルコース軌跡を描いていただきます。得点対象の課題の前に、任意の練習ラウンドが2回用意されます。",
        "各課題の後、実際のグルコース値が表示され、精度指標が算出されます。最後に、歴史的データから判定されたご自身の予測精度の要約と、パーセンタイル順位をお受け取りいただきます。",
        "予測精度は次の4群で比較されます。糖尿病がありCGMを使用している人、糖尿病がありCGMを使用していない人、糖尿病がなくCGMを使用している人、および糖尿病がなくCGMの経験がない人です。冒頭で収集したデータに基づき、これらの群のいずれかに割り当てられます。",
        "ご自身のCGMデータの任意アップロード",
        "ご自身でCGM装置を使用されている場合、任意でエクスポートファイル（CSVまたはJSON）をアップロードできます。そこから短い区間が抽出され、追加の予測課題に用いられます。これにより、歴史的データよりもご自身のデータをより正確に予測できるかどうかを比較できます。日付および時刻はユーザーインターフェースには表示されません。アップロードされた生ファイルは30日以内に削除され、抽出された区間のみが仮名化された形で保持されます。",
        "任意参加",
        "本研究への参加は任意です。同意を表明された場合にのみ組み入れられます。研究に参加されない場合、または後に撤回を希望される場合でも、不利益を受けることはありません。理由を述べることなく、いつでも参加を終了できます。完了したラウンドの部分的なデータは、有効なラウンドが少なくとも6回完了している場合にのみ含められます。",
        "誰が参加できますか。",
        "インターネットにアクセスできる18歳以上の成人が参加できます。糖尿病のある方もない方も対象です。",
        "参加に伴う可能性のあるリスク、不快感、または副作用は何ですか。",
        "本研究はデータの収集のみを行うため、参加に伴う医学的リスクはありません。精度の値は研究目的にのみ関わり、診断的または臨床的な意味はありません。",
        "研究への参加から得られる可能性のある利益は何ですか。",
        "参加によってご自身の健康上の利益はありません。参加は、人のグルコース値予測能力に関する体系的なベンチマークに寄与します。個人の精度要約および結果カードをお受け取りいただきます。金銭的な報酬はありません。",
        "参加に伴う権利および条件は何ですか。",
        "理由を述べなくても、いつでも研究への参加を終了できます。参加を希望されない場合、または撤回を希望される場合でも、不利益を受けることはありません。",
        "本プロジェクトおよび付随する研究については、医学的処置を伴わない純粋なオンライン研究でありリスクが最小限であるため、別途の保険は付保されていないことにご留意ください。",
        "本研究は、所轄の倫理委員会から肯定的な見解を得ています。",
        "データ保護",
        "以下では、研究の範囲内でお客様の個人データがどのように扱われ、私たちがどのように取り扱うかを、包括的にお知らせします。",
        "1. データの処理および保管の責任者は誰ですか。",
        "ロストック大学医学部が、研究の範囲内におけるデータの処理および保管の責任を負います。HEALESは、ロストック大学医学部とのデータ処理契約（DPA）の枠組みにおいて、処理者として Sugar-Sugar アプリケーションを運用します。責任機関には、次の連絡先でご連絡いただけます。",
        "責任機関：ロストック大学医学部 — ロストック大学の法人格を有する下部機関",
        "Schillingallee 35, 18057 Rostock",
        "www.med.uni-rostock.de",
        "2. 研究またはデータ保護に関する質問は、誰に連絡できますか。",
        "お客様の利益を最大限に守るため、研究への参加または手続全般に関するご質問は、いつでも以下の者にもご連絡いただけます。",
        "研究連絡先：",
        "• Anton Kulaga: anton.kulaga@uni-rostock.de",
        "• Livia Zaharia: livia.zaharia@uni-rostock.de",
        "データ保護に関するご質問については、ロストック大学医学部のデータ保護責任者にもご連絡いただけます。",
        "Doberaner Str. 142, 18057 Rostock",
        "0381 494 5155 | datenschutz@med.uni-rostock.de",
        "違法なデータ処理について苦情を申し立てる権利を行使される場合は、所轄の監督当局にご連絡ください。",
        "メクレンブルク＝フォアポンメルン州データ保護・情報公開担当委員",
        "Schloss Schwerin, Lennéstr. 1, 19053 Schwerin",
        "info@datenschutz-mv.de",
        "3. 研究の実施のために、どのデータが必要ですか。",
        "研究の範囲内で、次のデータが収集されます。",
        "連絡先：電子メールアドレス（登録、撤回の取扱い、および任意の再通知のため）。",
        "人口統計データ：年齢、性別、居住国。任意：体重および身長。",
        "健康関連データ：糖尿病の状態（有無、型、期間）、CGMの使用歴。",
        "研究データ：予測されたグルコース値および該当する場合は実際のグルコース値、精度指標（MAE、MSE、RMSE、MAPE）。",
        "同意に関する選択：同意に関するご判断の記録。",
        "任意 — ご自身のCGMデータ：アップロードされた場合、抽出された区間のみが仮名化された形で保持されます。",
        "4. これらのデータはどこから取得しますか。",
        "すべてのデータは、参加者であるお客様から、Sugar-Sugar ウェブアプリケーションへの入力（登録用質問票、予測課題、任意のCGMアップロード）を通じて、直接かつ専ら収集されます。他の情報源からデータを取得することはありません。処理の法的根拠は、Art. 6(1)(a) GDPR および Art. 9(2)(a) GDPR に基づくお客様の明示的な同意です。",
        "5. どの目的でデータが必要であり、どのように保護されますか。",
        "上記のデータは、人のグルコース予測能力に関する科学的調査のためにのみ必要であり、この目的にのみ用いられます。",
        "各参加者は、登録時に無作為の研究IDを受け取ります。すべての研究データは、専らこのIDに結び付けられます。研究IDと電子メールアドレスを結びつける別個の暗号化された対応ファイルは別に保管され、研究責任者（Anton Kulaga および Livia Zaharia）のみがアクセスできます。このファイルは、撤回の請求および任意の結果通知の取扱いにのみ用いられます。したがって、他の研究スタッフがお客様を識別することはできません。",
        "データは、送信中および保管中に暗号化されます。保管は欧州連合内のサーバ（GDPRに適合したホスティング）で行われます。アクセスは役割に基づいて管理されます。",
        "6. データはどこに、どのくらいの期間保管されますか。",
        "研究の範囲内で収集されたデータは、まず仮名化されます。対応ファイル（電子メール — 研究ID）は、研究終了から12か月後に破棄される見込みです。その後、研究データは匿名化され、すなわちお客様の身元に関するいかなる推論もできなくなります。",
        "仮名化された研究データは、ドイツの研究基準に従い10年間保持されます。アップロードされた生のCGMファイルは30日以内に削除されます。",
        "7. 誰がお客様のデータを知ることになりますか。データは共有または公表されますか。",
        "仮名化された研究データにアクセスできるのは、IBIMAの研究チームのみです。対応ファイルにアクセスできるのは、研究責任者（Anton Kulaga および Livia Zaharia）のみです。研究結果は、集計され匿名化された形でのみ公表されます。公表物から個人を推論することはできません。",
        "8. 研究に参加する場合、どのようなデータ保護上の権利がありますか。",
        "適用されるデータ保護規則（GDPR）に従い、保管されているデータへのアクセス、不正確なデータの訂正、データの消去、処理の制限、不当な処理に対する異議、およびお客様について保管されているデータのデータポータビリティの権利を有します。また、同意を撤回する権利もあります。これらの権利は、いつでも責任機関に対して行使できます。",
        "撤回権",
        "原則として、理由を述べることなく、いつでも同意を撤回できます。研究責任者（anton.kulaga@uni-rostock.de）までご連絡ください。撤回は、提出された時点から効力を生じます。その時点までのお客様のデータの処理は適法なままです。撤回の場合、個人データの削除が開始されます。すでに匿名化され、したがってお客様と結び付けることができなくなった研究データは、研究の範囲内で引き続き用いられます。対応ファイルは研究終了から12か月後に破棄される見込みであり、その後は個別の対応付け、したがって削除はもはや不可能です。",
        "法的根拠：Art. 6(1)(a) GDPR および Art. 9(2)(a) GDPR。",
        "Sugar-Sugar グルコース予測研究への参加およびそれに伴うデータ処理に関する同意宣言",
        "参加者の研究ID：_________________________________（登録時に割り当て）",
        "以下の同意は、登録時に該当する欄にチェックを入れることにより、電子的に記録されます。",
        "私は任意に本研究に参加します。本研究の参加者情報、それに含まれるデータ保護に関する案内、および関連する権利を注意深く読みました。",
        "研究の範囲内で個人データ、とりわけ私の健康データ（糖尿病の状態、CGMの使用歴、予測成績）が収集、処理および使用されることについて、説明を受けました。",
        "参加者情報に記載のとおり、処理はロストック大学医学部IBIMAの研究チームにより仮名化された形で行われます。HEALESはDPAに基づく処理者として関与します。",
        "データが匿名で保管される場合、私の請求による削除はもはや不可能であることを承知しています。",
        "撤回権",
        "理由を述べることなく、いつでも参加を終了でき、私に不利益が生じないことについて、説明を受けました。また、将来に向けていつでもデータ処理への同意を撤回でき、撤回までの同意に基づいて行われた処理の適法性には影響しないことも承知しています。該当する参加者情報における同意撤回に関する案内を確認しました。",
        "私はここに、Sugar-Sugar グルコース予測研究への任意の参加に同意します。理由を述べることなく、いつでも参加を終了でき、私に不利益が生じないことを理解しています。",
        "私はここに、該当する参加者情報を含め、以下の目的のために私の個人データおよび健康データを処理することに同意します。",
        "☐  参加者情報に列挙された私のデータ、とりわけ健康データ（糖尿病の状態、CGMの使用歴、予測成績）の収集、処理および使用を、ロストック大学医学部IBIMAの研究チームならびに処理者としてのHEALESが研究実施のために仮名化された形で行うこと。保管は欧州連合内のサーバで行われます。研究データは、対応ファイルの匿名化後最大10年間保持されます。撤回は研究終了後12か月まで可能であり、その後は識別ができなくなります。法的根拠：Art. 6(1)(a) GDPR および Art. 9(2)(a) GDPR。",
        "☐  参加者は18歳以上であることを確認します。",
        "任意の同意（拒否しても研究への参加には影響しません）：",
        "☐  ご自身のデータを用いる任意の予測課題のため、私自身のCGMデータをアップロードすることに同意します。生データは30日以内に削除され、仮名化された抽出区間のみが保持されます。",
        "☐  公表後、登録した電子メールアドレスを通じて研究チームから研究結果について通知を受けることに同意します。",
        "HEALESまたは関連プロジェクトからの通知は、研究終了後に別ページで提示され、研究への参加および同意とは完全に独立しています。",
        "本研究は、ロストック大学医学部倫理委員会の承認を受けています。登録番号：Ref. number A 2026-0064。倫理委員会による受領日：2026年2月27日。",
        "ご質問または撤回のご請求：anton.kulaga@uni-rostock.de | livia.zaharia@uni-rostock.de",
    ],
    "ko": [
        "참여자 정보",
        "이전 값 패턴에 기반한 혈당 궤적 예측: 온라인 벤치마킹 연구",
        "약칭: Sugar-Sugar 혈당 예측 연구",
        "결정을 내리기 전에 다음 글을 주의 깊게 읽어 주십시오. 질문이 있으시면 연구 책임자에게 연락해 주십시오.",
        "참여자님께,",
        "과학 연구에 참여하실 의사가 있는지 여쭙고자 합니다. 본 연구는 CGM 데이터(연속혈당측정 장치에서 얻은 데이터)를 바탕으로 사람이 단기 혈당 궤적을 얼마나 정확히 예측할 수 있는지를 처음으로 체계적으로 조사하는 것을 목표로 합니다. 현재까지 그러한 벤치마크는 없습니다. 귀하의 참여는 사람의 예측 정확도에 대한 참조값을 확립하는 데 도움이 되며, 이 값은 장기적으로 컴퓨터 기반 예측 모델을 평가하는 데에도 사용될 수 있습니다. 이러한 예측 모델은 당뇨병 연구와 당뇨병 진료에서 점점 더 많이 사용되고 있으며, 여기에서 수집된 데이터를 통해 타당성을 검증하고 비교할 수 있습니다.",
        "본 연구는 로스토크 대학의학의 IBIMA(의학 및 노화 연구 생물통계·정보학 연구소)에서, 연구의 재원 제공자인 HEALES(Healthy Life Extension Society, 브뤼셀)와 협력하여 수행됩니다. 연구 책임자는 Anton Kulaga(로스토크 대학의학)와 Livia Zaharia(HEALES)입니다. 최소 200명의 참여자가 계획되어 있습니다.",
        "연구 참여는 어떠한 의학적 치료에도 영향을 주지 않습니다. 대면 예약이나 의학적 처치가 없는 순수 온라인 연구입니다.",
        "연구는 어떻게 진행됩니까?",
        "참여는 Sugar-Sugar 웹 애플리케이션을 통해 전적으로 온라인으로 이루어지며, 약 15–20분이 소요됩니다.",
        "먼저 짧은 등록 설문(나이, 성별, 거주 국가, 당뇨병 상태, CGM 사용 이력; 선택: 체중 및 신장)을 작성하게 됩니다. 그다음 6건에서 12건의 예측 과제를 수행합니다. 각 과제는 공개적으로 알려진 익명화 데이터셋에서 가져온 과거 3시간 분량의 CGM 데이터를 보여 줍니다. 다른 연구에서 수집된 시계열의 앞선 시점들이 제시되며, 이후 시점을 추정하도록 요청받습니다. 차트에 12개의 점을 찍어 이어지는 60분 동안의 예상 혈당 궤적을 그리게 됩니다. 점수가 매겨지는 과제 전에 선택적인 연습 라운드 두 번이 제공됩니다.",
        "각 과제 후에 실제 혈당 값이 표시되고 정확도 지표가 계산됩니다. 마지막에 역사적 데이터로부터 산출된 귀하의 예측 정확도 요약과 백분위 순위를 받게 됩니다.",
        "예측 정확도는 네 집단에 걸쳐 비교됩니다. 당뇨병이 있고 CGM을 사용하는 사람, 당뇨병이 있고 CGM을 사용하지 않는 사람, 당뇨병이 없고 CGM을 사용하는 사람, 당뇨병이 없고 CGM 경험이 없는 사람입니다. 처음에 수집된 데이터를 바탕으로 이 집단 중 하나에 배정됩니다.",
        "본인 CGM 데이터의 선택적 업로드",
        "본인이 CGM 기기를 사용하는 경우, 선택적으로 내보내기 파일(CSV 또는 JSON)을 업로드할 수 있습니다. 여기에서 짧은 구간이 추출되어 추가 예측 과제에 사용됩니다. 이를 통해 역사적 데이터보다 본인 데이터를 더 정확히 예측하는지를 비교할 수 있습니다. 날짜와 시각은 사용자 인터페이스에 표시되지 않습니다. 업로드된 원본 파일은 30일 이내에 삭제되며, 추출된 구간만 가명처리된 형태로 보존됩니다.",
        "자발적 참여",
        "본 연구에의 참여는 자발적입니다. 동의를 밝힌 경우에만 포함됩니다. 연구에 참여하지 않거나 나중에 철회하고자 하더라도 불이익을 받지 않습니다. 이유를 밝히지 않고 언제든지 참여를 종료할 수 있습니다. 완료된 라운드의 부분 데이터는 유효한 라운드를 최소 6회 완료한 경우에만 포함됩니다.",
        "누가 참여할 수 있습니까?",
        "인터넷에 접속할 수 있는 만 18세 이상의 성인이 참여할 수 있습니다. 당뇨병이 있는 사람과 없는 사람 모두 대상입니다.",
        "참여와 관련된 가능한 위험, 불편 또는 부작용은 무엇입니까?",
        "본 연구는 데이터만 수집하므로 참여와 관련된 의학적 위험은 없습니다. 정확도 값은 연구 목적으로만 의미가 있으며 진단적 또는 임상적 의미는 없습니다.",
        "연구 참여로부터 얻는 가능한 이익은 무엇입니까?",
        "참여로 인한 개인적인 건강상 이익은 없습니다. 참여는 사람의 혈당 값 예측 능력에 대한 체계적 벤치마크에 기여합니다. 개인 정확도 요약과 결과 카드를 받게 됩니다. 금전적 보상은 없습니다.",
        "참여와 관련된 권리와 조건은 무엇입니까?",
        "이유를 밝히지 않아도 언제든지 연구 참여를 종료할 수 있습니다. 참여를 원하지 않거나 철회하고자 하더라도 불이익을 받지 않습니다.",
        "본 프로젝트와 그에 따르는 연구는 의학적 처치가 없는 순수 온라인 연구로 위험이 최소이므로, 별도의 보험이 가입되어 있지 않음을 유의해 주십시오.",
        "본 연구는 관할 윤리위원회로부터 긍정적 의견을 받았습니다.",
        "개인정보 보호",
        "아래에서는 연구 범위 내에서 귀하의 개인정보가 어떻게 다루어지며 우리가 이를 어떻게 취급하는지를 포괄적으로 알려 드리고자 합니다.",
        "1. 데이터 처리 및 보관의 책임자는 누구입니까?",
        "로스토크 대학의학이 연구 범위 내 데이터 처리 및 보관에 대한 책임을 집니다. HEALES는 로스토크 대학의학과의 데이터 처리 계약(DPA) 틀 안에서 처리자로서 Sugar-Sugar 애플리케이션을 운영합니다. 책임 기관에는 다음 연락처로 연락하실 수 있습니다.",
        "책임 기관: 로스토크 대학의학 — 로스토크 대학교의 법인격을 갖춘 하부 기관",
        "Schillingallee 35, 18057 Rostock",
        "www.med.uni-rostock.de",
        "2. 연구 또는 개인정보 보호에 관한 질문은 누구에게 할 수 있습니까?",
        "귀하의 이익을 최대한 보호하기 위해, 연구 참여 또는 전반적인 절차에 관한 질문은 언제든지 다음 사람에게도 연락하실 수 있습니다.",
        "연구 연락처:",
        "• Anton Kulaga: anton.kulaga@uni-rostock.de",
        "• Livia Zaharia: livia.zaharia@uni-rostock.de",
        "개인정보 보호 관련 질문은 로스토크 대학의학의 개인정보 보호 책임자에게도 문의하실 수 있습니다.",
        "Doberaner Str. 142, 18057 Rostock",
        "0381 494 5155 | datenschutz@med.uni-rostock.de",
        "위법한 데이터 처리에 대해 불만을 제기할 권리를 행사하고자 하시면 관할 감독 기관에 연락해 주십시오.",
        "메클렌부르크-포어포메른 주 개인정보 보호 및 정보공개 담당 위원",
        "Schloss Schwerin, Lennéstr. 1, 19053 Schwerin",
        "info@datenschutz-mv.de",
        "3. 연구 수행을 위해 어떤 데이터가 필요합니까?",
        "연구 범위 내에서 다음 데이터가 수집됩니다.",
        "연락처: 이메일 주소(등록, 철회 처리 및 선택적 재알림용).",
        "인구통계 데이터: 나이, 성별, 거주 국가; 선택: 체중 및 신장.",
        "건강 관련 데이터: 당뇨병 상태(유무, 유형, 기간), CGM 사용 이력.",
        "연구 데이터: 예측된 혈당 값과 해당하는 경우 실제 혈당 값, 정확도 지표(MAE, MSE, RMSE, MAPE).",
        "동의 선택: 귀하의 동의 결정에 대한 기록.",
        "선택 — 본인 CGM 데이터: 업로드된 경우 추출된 구간만 가명처리된 형태로 보존됩니다.",
        "4. 이 데이터는 어디에서 얻습니까?",
        "모든 데이터는 참여자인 귀하로부터, Sugar-Sugar 웹 애플리케이션에 대한 입력(등록 설문, 예측 과제, 선택적 CGM 업로드)을 통해 직접적이고 전적으로 수집됩니다. 다른 출처에서 데이터를 얻지 않습니다. 처리의 법적 근거는 Art. 6(1)(a) GDPR 및 Art. 9(2)(a) GDPR에 따른 귀하의 명시적 동의입니다.",
        "5. 어떤 목적으로 데이터가 필요하며 어떻게 보호됩니까?",
        "위에 열거된 데이터는 사람의 혈당 예측 능력에 대한 과학적 조사를 위해서만 필요하며 이 목적으로만 사용됩니다.",
        "각 참여자는 등록 시 무작위 연구 ID를 받습니다. 모든 연구 데이터는 전적으로 이 ID에 연결됩니다. 연구 ID와 이메일 주소를 연결하는 별도의 암호화된 매핑 파일은 따로 보관되며 연구 책임자(Anton Kulaga 및 Livia Zaharia)만 접근할 수 있습니다. 이 파일은 철회 요청과 선택적 결과 알림을 처리하는 데에만 사용됩니다. 따라서 다른 연구 직원은 귀하를 식별할 수 없습니다.",
        "데이터는 전송 중 및 저장 중에 암호화됩니다. 보관은 유럽연합 내 서버(GDPR 준수 호스팅)에서 이루어집니다. 접근은 역할에 따라 규제됩니다.",
        "6. 데이터는 어디에, 얼마나 오래 보관됩니까?",
        "연구 범위 내에서 수집된 데이터는 먼저 가명처리됩니다. 매핑 파일(이메일 – 연구 ID)은 연구 종료 후 12개월에 파기될 것으로 예상됩니다. 그 후 연구 데이터는 익명화되어, 즉 귀하의 신원에 대한 어떠한 추론도 더 이상 허용하지 않습니다.",
        "가명처리된 연구 데이터는 독일 연구 기준에 따라 10년간 보존됩니다. 업로드된 원본 CGM 파일은 30일 이내에 삭제됩니다.",
        "7. 누가 귀하의 데이터를 알게 됩니까? 데이터가 공유되거나 공개됩니까?",
        "가명처리된 연구 데이터는 IBIMA의 연구팀만 접근할 수 있습니다. 매핑 파일은 연구 책임자(Anton Kulaga 및 Livia Zaharia)만 접근할 수 있습니다. 연구 결과는 집계되고 익명화된 형태로만 공개됩니다. 출판물로부터 개인을 추론하는 것은 불가능합니다.",
        "8. 연구에 참여할 때 어떤 개인정보 보호 권리가 있습니까?",
        "적용되는 개인정보 보호 규정(GDPR)에 따라 귀하는 저장된 데이터에 대한 열람권, 부정확한 데이터의 정정, 데이터의 삭제, 처리의 제한, 부당한 처리에 대한 이의, 그리고 귀하에 관해 저장된 데이터의 이동권을 갖습니다. 또한 동의를 철회할 권리도 있습니다. 이러한 권리는 언제든지 책임 기관에 대해 행사할 수 있습니다.",
        "철회권",
        "원칙적으로 이유를 밝히지 않고 언제든지 동의를 철회할 수 있습니다. 연구 책임자(anton.kulaga@uni-rostock.de)에게 연락해 주십시오. 철회는 제출한 시점부터 효력이 발생합니다. 그 시점까지의 데이터 처리는 적법한 상태로 남습니다. 철회 시 개인정보의 삭제가 개시됩니다. 이미 익명화되어 더 이상 귀하와 연결할 수 없는 연구 데이터는 연구 범위 내에서 계속 사용됩니다. 매핑 파일은 연구 종료 후 12개월에 파기될 것으로 예상되며, 그 이후에는 개별 대응 및 따라서 삭제가 더 이상 불가능합니다.",
        "법적 근거: Art. 6(1)(a) GDPR 및 Art. 9(2)(a) GDPR.",
        "Sugar-Sugar 혈당 예측 연구 참여 및 관련 데이터 처리에 대한 동의 선언",
        "참여자의 연구 ID: _________________________________ (등록 시 부여)",
        "다음 동의는 등록 시 해당 칸에 표시함으로써 전자적으로 기록됩니다.",
        "본인은 자발적으로 본 연구에 참여합니다. 본 연구의 참여자 정보와 그에 포함된 개인정보 보호 안내 및 관련 권리를 주의 깊게 읽었습니다.",
        "연구 범위 내에서 개인정보, 특히 본인의 건강 데이터(당뇨병 상태, CGM 사용 이력, 예측 성적)가 수집, 처리 및 사용된다는 설명을 들었습니다.",
        "참여자 정보에 명시된 바와 같이, 처리는 로스토크 대학의학 IBIMA 연구팀에 의해 가명처리된 형태로 이루어집니다. HEALES는 DPA에 따른 처리자로 활동합니다.",
        "데이터가 익명으로 저장되는 경우 본인의 요청에 따른 삭제가 더 이상 불가능함을 알고 있습니다.",
        "철회권",
        "이유를 밝히지 않고 언제든지 참여를 종료할 수 있으며 본인에게 불이익이 없다는 설명을 들었습니다. 또한 장래에 대하여 언제든지 데이터 처리 동의를 철회할 수 있으며, 철회 시점까지 동의에 근거하여 수행된 처리의 적법성에는 영향을 주지 않음을 알고 있습니다. 해당 참여자 정보의 동의 철회에 관한 안내를 확인하였습니다.",
        "본인은 이에 Sugar-Sugar 혈당 예측 연구에 자발적으로 참여하는 데 동의합니다. 이유를 밝히지 않고 언제든지 참여를 종료할 수 있으며 본인에게 불이익이 없음을 이해합니다.",
        "본인은 이에 해당 참여자 정보를 포함하여, 다음 목적을 위해 본인의 개인정보 및 건강 데이터를 처리하는 데 동의합니다.",
        "☐  참여자 정보에 열거된 본인의 데이터, 특히 건강 데이터(당뇨병 상태, CGM 사용 이력, 예측 성적)의 수집, 처리 및 사용을, 로스토크 대학의학 IBIMA 연구팀과 처리자로서의 HEALES가 연구 수행을 위해 가명처리된 형태로 하는 것. 보관은 유럽연합 내 서버에서 이루어집니다. 연구 데이터는 매핑 파일 익명화 후 최대 10년간 보존됩니다. 철회는 연구 종료 후 12개월까지 가능하며, 그 이후에는 식별이 더 이상 불가능합니다. 법적 근거: Art. 6(1)(a) GDPR 및 Art. 9(2)(a) GDPR.",
        "☐  참여자는 만 18세 이상임을 확인합니다.",
        "선택 동의(거부는 연구 참여에 영향을 주지 않습니다):",
        "☐  본인 데이터를 사용하는 선택 예측 과제를 위해 본인의 CGM 데이터를 업로드하는 데 동의합니다. 원본 데이터는 30일 이내에 삭제되며, 가명처리된 추출 구간만 보존됩니다.",
        "☐  공개 후 등록된 이메일 주소를 통해 연구팀으로부터 연구 결과에 대해 안내받는 데 동의합니다.",
        "HEALES 또는 관련 프로젝트의 알림은 연구 종료 후 별도의 페이지에 제시되며, 연구 참여 및 동의와는 완전히 독립적입니다.",
        "본 연구는 로스토크 대학의학 윤리위원회의 승인을 받았습니다. 등록 번호: Ref. number A 2026-0064. 윤리위원회 접수일: 2026년 2월 27일.",
        "질문 또는 철회 요청: anton.kulaga@uni-rostock.de | livia.zaharia@uni-rostock.de",
    ],
}
...
def consent_notice_children(
    ...
    *,
    ...
    replacements = {
        ...
        "bg": {
            "Информация за участника": "### Информация за участника",
            "Защита на данните": "### Защита на данните",
            "Право на оттегляне": "### Право на оттегляне",
        },
        "ja": {
            "参加者情報": "### 参加者情報",
            "データ保護": "### データ保護",
