...
_CONSENT_NOTICE_TRANSLATIONS: Final[dict[str, list[str]]] = {
    ...
    "ja": [
        "参加者へのご案内",
        "過去のデータパターンに基づく血糖変動の予測：オンラインベンチマーク研究",
        "（略称：Sugar-Sugar 血糖予測研究）",
        "ご参加を検討される前に、以下の説明文書をよくお読みください。ご不明な点がございましたら、研究責任者までお問い合わせください。",
        "参加者の皆様へ",
        "学術研究へのご協力のお願いです。本研究は、持続血糖測定器（CGM）のデータに基づき、人間が短期的な血糖変動をどれほど正確に予測できるかを世界で初めて体系的に調査することを目的としています。現在、このような基準値（ベンチマーク）は存在していません。ご参加いただくことで、人間の予測精度の基準値が確立され、将来的にはAIなどのコンピュータ予測モデルの評価にも活用されます。これらの予測モデルは糖尿病研究や医療現場での活用が進んでおり、本研究で収集されたデータによってモデルの検証・比較が可能となります。",
        "本研究は、ロストック大学医療センター（ドイツ）の「医用生体統計・情報学・加齢研究研究所（IBIMA）」が主体となり、本研究の資金提供者である「HEALES（ベルギー・健康長寿推進協会）」との共同研究として実施されます。研究責任者は Anton Kulaga（ロストック大学医療センター）および Livia Zaharia（HEALES）です。少なくとも200名の参加者を予定しています。",
        "本研究への参加は、いかなる医療行為や治療にも影響を与えません。対面での受診や医療処置を伴わない、完全オンラインの研究です。",
        "研究の流れ",
        "参加はすべて「Sugar-Sugar」ウェブアプリ上で行われ、所要時間は約15〜20分です。",
        "まず、簡単な登録アンケート（年齢、性別、居住国、糖尿病の有無、CGM使用歴。任意項目：体重・身長）にご回答いただきます。続いて、6〜12回の予測タスクを行います。各タスクでは、公表されている匿名化データセットから抽出された3時間分の過去のCGMデータが表示されます。過去の研究で収集されたデータのうち、前半の時系列データを見ながら、後半（続く60分間）の血糖値の動きを予想してグラフ上に12個の点を配置して描きます。なお、採点対象のタスクに入る前に、2回の練習ラウンド（任意）を体験できます。",
        "各タスクの終了後、実際の血糖値が表示され、予測の精度指標が計算されます。すべてのタスクが終了すると、ご自身の予測精度の要約とパーセンタイル順位（全体における相対位置）が表示されます。",
        "予測精度は以下の4つのグループで比較されます：糖尿病があり、CGM使用経験がある方、糖尿病があり、CGM使用経験がない方、糖尿病がなく、CGM使用経験がある方、糖尿病がなく、CGM使用経験がない方。最初のアンケートでご回答いただいた内容に基づき、上記のいずれかのグループに割り当てられます。",
        "ご自身のCGMデータの任意アップロード",
        "ご自身でCGM機器をお使いの場合、オプションでエクスポートファイル（CSVまたはJSON）をアップロードできます。ファイルから短いデータセクションが抽出され、追加の予測タスクに使用されます。これにより、ご自身のデータと過去の他者データを比べることで、自分自身の血糖変動のほうがより正確に予測できるかを検証します。画面上に日時などの個人情報は表示されません。アップロードされた生ファイルは30日以内に削除され、抽出されたデータセクションのみが仮名化（匿名化に近い状態）された状態で保管されます。",
        "自由意思による参加",
        "本研究への参加は完全に自由です。同意いただいた場合のみ研究対象となります。参加を取りやめたり、後から撤回（取り消し）した場合でも、不利益を被ることは一切ありません。理由を明かすことなく、いつでも参加を終了できます。なお、有効なタスクを6回以上完了した場合のみ、一部完了したデータとして解析に含まれます。",
        "参加対象者",
        "インターネットにアクセスできる18歳以上の成人が対象です。糖尿病の有無にかかわらずご参加いただけます。",
        "想定されるリスクや不快感について",
        "本研究はデータ収集のみを行うため、医療上のリスクや副作用はありません。算出される予測精度は純粋に学術研究目的のものであり、診断や臨床上の意味を持つものではありません。",
        "参加によるメリット",
        "本研究に参加することによる個人的な健康上の直接的メリットはありません。ただし、人間の血糖予測能力に関する体系的な基準値の確立に寄与することができます。参加後には、個人の予測精度要約と結果カードが提供されます。金銭的な謝礼（対価）はありません。",
        "参加に伴う権利と条件",
        "理由を告げることなく、いつでも研究参加を終了できます。参加しない場合や途中で参加を撤回した場合でも、いかなる不利益も生じません。",
        "なお、本研究は医療処置を伴わない完全オンライン研究であり、リスクが極めて小さいため、本プロジェクト専用の特別な傷害保険等には加入しておりません。",
        "本研究は、担当の倫理委員会から適切な研究として承認を得ています。",
        "個人情報の保護（プライバシーポリシー）",
        "本研究における個人データの取り扱いおよび保護について、以下の通り詳細をご案内いたします。",
        "1. データ処理および保管の責任者は誰ですか？",
        "本研究における個人データの処理および保管の責任主体はロストック大学医療センターです。HEALESは、ロストック大学医療センターとのデータ処理契約（DPA）に基づき、Sugar-Sugarアプリケーションの運営事業者としてデータを処理します。",
        "【データ管理責任者】ロストック大学医療センター（University Medicine Rostock）",
        "Schillingallee 35, 18057 Rostock, Germany",
        "www.med.uni-rostock.de",
        "2. 研究やデータ保護に関する問い合わせ先はどこですか？",
        "研究への参加や手続きについてご質問がある場合は、いつでも以下の研究責任者までお問い合わせください。",
        "【研究責任者】",
        "• Anton Kulaga: anton.kulaga@uni-rostock.de",
        "• Livia Zaharia: livia.zaharia@uni-rostock.de",
        "データ保護に関するご質問は、ロストック大学医療センターのデータ保護役員（DPO）宛にもお問い合わせいただけます。",
        "住所: Doberaner Str. 142, 18057 Rostock",
        "電話: +49 0381 494 5155 | Eメール: datenschutz@med.uni-rostock.de",
        "不適切なデータ処理に対して苦情申し立ての権利を行使される場合は、管轄の監督機関までご連絡ください。",
        "メクレンブルク＝フォアポンメルン州 データ保護・情報公開法務監察官",
        "住所: Schloss Schwerin, Lennéstr. 1, 19053 Schwerin",
        "Eメール: info@datenschutz-mv.de",
        "3. 研究の実施に必要なデータは何ですか？",
        "本研究では以下のデータを収集します。",
        "連絡先情報: Eメールアドレス（アカウント登録、参加撤回の処理、希望者への結果通知のため）",
        "属性データ: 年齢、性別、居住国、（任意：体重、身長）",
        "健康関連データ: 糖尿病の有無（種類、罹患期間）、CGMの使用歴",
        "研究データ: 予測された血糖値、実際の血糖値、予測精度指標（MAE、MSE、RMSE、MAPEなど）",
        "同意状況: 選択された同意内容の記録",
        "（任意）自身のCGMデータ: アップロードされた場合、仮名化された抽出セクションのみが保存されます",
        "4. データの取得方法と法的根拠は何ですか？",
        "すべてのデータは、Sugar-Sugarウェブアプリ上の入力（登録アンケート、予測タスク、任意のCGMアップロード）を通じて、参加者ご本人から直接収集されます。第三者からデータを取得することはありません。データ処理の法的根拠は、EU一般データ保護規則（GDPR）第6条第1項(a)および第9条第2項(a)に基づく、参加者ご本人の明示的な同意です。",
        "5. データの利用目的と保護方法について",
        "収集されたデータは、人間の血糖予測能力に関する学術的調査のためだけに利用され、その他の目的には一切使用されません。",
        "登録時に各参加者にはランダムな研究IDが発行されます。すべての研究データはこの研究IDのみに紐づけられます。Eメールアドレスと研究IDを紐づける対応表（暗号化ファイル）は研究データとは切り離して保管され、研究責任者（Anton KulagaおよびLivia Zaharia）のみがアクセスできます。この対応表は、参加撤回の申請処理および結果の通知にのみ使用されます。そのため、他の研究スタッフが個人を特定することはできません。",
        "データは通信時および保存時に暗号化されます。データはEU域内のサーバー（GDPR準拠のホスティング）に安全に保管され、アクセス権限は厳格に管理されます。",
        "6. データの保管場所と保管期間について",
        "収集されたデータはまず仮名化（個人識別情報をIDに置換）されます。Eメールアドレスと研究IDの対応表は、研究完了の12ヶ月後に破棄される予定です。対応表の破棄後はデータが完全匿名化され、個人を特定することは不可能になります。",
        "仮名化された研究データは、ドイツの研究倫理基準に従い10年間保存されます。アップロードされた生CGMファイルは30日以内に削除されます。",
        "7. データの共有および公表について",
        "仮名化された研究データにアクセスできるのは、ロストック大学医療センターIBIMAの研究チームのみです。対応表にアクセスできるのは研究責任者のみです。研究結果は集計された統計データ（匿名化データ）としてのみ論文等で公表されます。公表資料から個人が特定されることはありません。",
        "8. 参加者が持つデータ保護上の権利",
        "GDPRに基づき、参加者は自身の保存データへのアクセス権、開示請求、不正確なデータの訂正、削除請求、処理の制限、不当な処理に対する異議申し立て、データポータビリティの権利を有します。また、いつでも同意を撤回する権利を有します。これらの権利は、研究責任者に対して行使することができます。",
        "同意撤回権について",
        "原則として、理由を開示することなく、いつでも同意を撤回することができます。研究責任者（anton.kulaga@uni-rostock.de）までご連絡ください。撤回は申請された時点から有効となり、撤回前に行われたデータ処理の合法性には影響しません。撤回が申し出られた場合、個人データの削除手続きが開始されます。ただし、すでに匿名化され個人と結びつけられない研究データについては、引き続き研究に使用されます。なお、研究完了の12ヶ月後に対応表が破棄された後は、個人の特定およびデータの削除が不可能となりますのでご了承ください。",
        "法的根拠: GDPR 第6条第1項(a) および 第9条第2項(a)",
        "Sugar-Sugar 血糖予測研究への参加およびデータ処理に関する同意書",
        "参加者研究ID: ___ （登録時に自動発行）",
        "以下の同意項目は、登録時にチェックボックスにチェックを入れることで電子的に記録されます。",
        "私は自由意思により本研究に参加します。本研究の参加者案内、個人情報保護（プライバシーポリシー）に関する説明、および自身に認められた権利について注意深く読み、理解しました。",
        "本研究において個人データ、特に健康に関するデータ（糖尿病の有無、CGM使用歴、予測パフォーマンス等）が収集、処理、利用されることについて説明を受けました。",
        "参加者案内にある通り、データはロストック大学医療センターIBIMAの研究チームによって仮名化された状態で処理され、HEALESが受託者（Processor）としてアプリを運営することを理解しました。",
        "データの匿名化が完了した後は、自身の申し出によるデータの削除ができなくなることを理解しています。",
        "同意撤回について",
        "私は、自分に不利益が生じることなく、理由を問わずいつでも研究への参加を終了できる旨の説明を受けました。また、いつでも将来に向けてデータ処理の同意を撤回できること、および撤回前に行われた処理の合法性が損なわれないことを理解しました。",
        "上記を踏まえ、私は Sugar-Sugar 血糖予測研究へ参加することに同意します。理由を告げることなく、いかなる不利益も受けることなく、いつでも参加を終了できることを理解しています。",
        "私は、参加者案内に従い、以下の目的で自身の個人データおよび健康データが処理されることに同意します：",
        "☐  本研究を実施するため、ロストック大学医療センターIBIMAの研究チームおよび受託者であるHEALESにより、参加者案内記載のデータ（特に糖尿病状態、CGM使用歴、予測結果などの健康データ）が仮名化された状態で収集・処理・利用されること。データはEU域内のサーバーに保管されます。研究データは対応表の匿名化後最大10年間保管されます。同意の撤回は研究完了後12ヶ月まで可能であり、その後は個人の特定が不能となることに同意します。（法的根拠: GDPR 第6条第1項(a)および第9条第2項(a)）",
        "☐  参加者は18歳以上であることを確認します。",
        "任意の同意項目（同意しなくても研究への参加に影響はありません）：",
        "☐  （任意）自身のCGMデータをアップロードし、自身のデータを使用した追加予測タスクに参加することに同意します。生データは30日以内に削除され、抽出・仮名化されたデータのみが保管されることを理解しています。",
        "☐  （任意）研究終了後、登録したEメールアドレス宛に研究結果の通知を受け取ることに同意します。",
        "※ HEALESまたは関連プロジェクトからの通知は研究終了後に別ページで案内され、本研究への参加および同意とは完全に独立しています。",
        "本研究は、ロストック大学医療センター倫理委員会の承認を受けています。受付番号: Ref. number A 2026-0064（受付日: 2026年2月27日）",
        "ご質問・同意撤回の連絡先: anton.kulaga@uni-rostock.de | livia.zaharia@uni-rostock.de",
    ],
    "ko": [
        "참여자 정보",
...
def consent_notice_children(
    ...
    *,
    ...
    replacements = {
        ...
        "ja": {
            "参加者へのご案内": "### 参加者へのご案内",
            "個人情報の保護（プライバシーポリシー）": "### 個人情報の保護（プライバシーポリシー）",
            "同意撤回権について": "### 同意撤回権について",
            "同意撤回について": "### 同意撤回について",
        },
        "ko": {
