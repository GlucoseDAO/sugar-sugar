...
# NOTE: the /final Share *button* is gone.  The share flow is part of /final
# itself: `create_final_layout` builds the record via `build_final_share_record`,
# persists it with `share_store.ensure_share` (content-addressed, so re-renders
# reuse the file), and renders `build_share_panel` eagerly.  /share/<id> remains
# the recipient-facing page, and the one navigation *to* it is the name card's
# "Save & Share" below.
...
def _share_id_for_session(
    ...
    *,
    ...
    Deliberately the *same* builder `/final` renders its embedded panel from, so a
    link a player sends and the panel they were looking at can never describe two
    different games.  Save & Share passes the **just-renamed** ``user_info``, so the
    card carries the name that was only typed a moment ago.
    """
    record: Optional[dict[str, Any]] = build_final_share_record(
        user_info, locale=normalize_locale(interface_language)
    )
    if record is None:
        return None
    return share_store.ensure_share(record)
