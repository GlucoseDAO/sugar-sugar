...
uv run download-cgmacros fetches the public PhysioNet CGMacros dataset into `data/cgmmacros/` (gitignored). Default extract skips meal photos; `--photos` keeps them, `--force` re-downloads, `--keep-zip` retains the 627 MB archive. The dataset is not stored in git or Git LFS.
