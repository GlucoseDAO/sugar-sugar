...
- **Two sibling Flask routes**, not Dash pages:
  - `GET /share/<id>/image.png` — `sugar_sugar.share_png.render_share_card_png_bytes` renders `build_share_card_figure` to a 1080×1080 PNG (kaleido when Chromium is available; matplotlib Agg fallback otherwise). Results are cached in `_SHARE_PNG_CACHE` so repeated loads don't respawn Chromium.
...
- **`kaleido`** is used for the primary share PNG (pixel-perfect card). Kaleido v1 does not bundle Chrome; run `uv run setup-chrome` (or `choreo_get_chrome`) on deploy, or rely on the matplotlib fallback in `share_png.py` (no Chrome, simplified layout). First kaleido render takes ~1 s; subsequent are served from `_SHARE_PNG_CACHE`. Do NOT hot-reload the server while kaleido is rendering — it can leave orphaned Chromium processes on Windows.
