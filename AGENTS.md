...
uv run download-bigideas fetches PhysioNet BIG IDEAs Dexcom + food logs into `data/bigideas/` (gitignored). Empatica ACC/BVP files are not downloaded. CGMacros is unused in Format A.
uv run download-d1namo fetches the public D1NAMO (Dubosson) T1D subset into `data/d1namo/` (gitignored). Default extract keeps meal photos; `--no-photos` skips JPEGs, `--force` re-downloads, `--keep-zip` retains the 252 MB archive. ECG/sensor zips are not downloaded.
...
Architecture in one paragraph: the static viewport meta is **`width=device-width`** (NOT the old forced `1280`), and it stays device-width on **every** page **including `/prediction`** — that route used to force `width=1280` for Plotly drawline, but 1280 overflowed/cropped fullscreen landscape, so it was dropped (landscape uses the real ~800-900px device width; portrait scrolls the chart horizontally). A clientside callback in `app.py` keeps the meta at device-width and stamps a `route-prediction` class on `<html>` so CSS can target that page. Detection is two-pronged: a server-side `_is_mobile_ua()` check on the Flask request User-Agent picks **separate mobile builders** in `display_page` / `update_on_language_change` (`StartupPageMobile` wizard, `LandingPageMobile`, `MobileNavBar` burger menu); the existing clientside callback still adds the `html.mobile-device` class (UA + `(pointer:coarse)` fallback) used to scope `assets/mobile.css`. Display-only pages (ending, final, share, faq, about, contact, demo) just reflow via CSS once `device-width` is active. Hard rules: **do not CSS-rotate the chart** (`transform: rotate(90deg)` breaks Plotly drawline touch mapping); `render_mobile_warning()` returns `None` and `mobile-warning` is a throwaway Output sink. **The portrait "rotate to draw" nag overlay was REMOVED** (second non-playable mode). Single mobile `/prediction` flowpath: immersive landscape CSS applies on rotation (gated `@media (orientation: landscape) and (pointer: coarse)` — NO `max-device-width`; uses `100dvh` not `100vh`), and fullscreen is entered by a user GESTURE (clientside, mobile-only) via the wizard **Start button** OR a persistent **"Fullscreen mode" button** on `/prediction`, each `requestFullscreen(documentElement)` + best-effort `screen.orientation.lock('landscape')` (works on Android after fullscreen, rejects-caught on iOS; the OLD "never use orientation.lock" rule is superseded). Fullscreen is an enhancement — `100dvh`/device-width landscape is playable without it. Use `uv run python scripts/mobile_shots.py` to screenshot every page.

**`html, body { min-width: 1280px }` in `assets/lang.css` is the master mobile kill-switch** — a leftover that pins the whole page to 1280; `mobile.css` releases it with `min-width:0 !important` on `html.mobile-device` (now **all** mobile pages; the `:not(.route-prediction)` carve-out was removed when `/prediction` went device-width). If mobile pages render desktop-width again, check this first. NB: 1280 has TWO sources (the `<meta viewport>` switch AND this `min-width` anchor) — release both or `/prediction` stays pinned (meta says device-width but `innerWidth` stays 1280).
...
- **HISTORICAL / obsolete:** `/prediction` used to force a `width=1280` meta + a `max-device-width:1024` gate on the immersive CSS, which needed a `clearDeviceMetricsOverride` + re-apply dance in the harness. **Gone now** — `/prediction` is device-width and the gate is removed, so emulate it like any other page. Old shots cropping the chart bottom = stale cache/CSS, not the meta trick. Still true: **Plotly only re-fits on a window `resize`** — call `Plotly.Plots.resize(gd)` on every `.js-plotly-plot` before capturing (a bare `resize` event races the layout).
...
- **Fullscreen/immersive entry must be a clientside callback fired by a GESTURE** (route-change callbacks are rejected). Two gesture buttons: wizard `start-button` + persistent "Fullscreen mode" button on `/prediction`; each `requestFullscreen(documentElement)` + best-effort `screen.orientation.lock('landscape')` (Android works, iOS rejects-caught) + `Plotly.Plots.resize`. Reuse the proven demo-video fullscreen path. Localize clientside button feedback via a `data-*` attr (`t()`-rendered), e.g. `data-copied-text`.
- **Landscape `/prediction` header chips are absolute-positioned — rebalance edges together.** Round (`left+width`), Units (`right+width`), Source (`left`/`right`-pinned). Shrinking Round/Units won't widen Source unless you also move Source's `left`/`right`. Append a final landscape `@media` override (mobile.css has near-duplicate blocks).
- **Secondary actions go on the between-rounds `/ending` page, not in-round `/prediction`** (no spare screen budget in-round; chart + controls fill `100dvh`). The cross-device "copy resume link" button lives on `/ending`.
- **Removing/renaming a clientside callback breaks open tabs** → stale tab POSTs the old id, server 500s `KeyError: Callback function not found`. Bump `DEPLOY_BUILD` + hard-refresh; it's a stale client, not a server bug.
- **`/prediction` is device-width now → size its elements at native px** (no 1280 scale-down; the old ~3.3× sizing trap is gone).
...
- **`/startup` callbacks must not reference the `consent-*` components.** A Dash callback fires only when every Input/State component is in the current layout. The five `consent-*` checkboxes live in `landing.py` and render on the desktop landing page and the mobile wizard step 0 — but never on desktop `/startup`. So any `/startup` callback that takes a `consent-*` as Input/State silently never fires on desktop: the Start button activates but goes nowhere, and form validation dies. (Bit `handle_start_button` and `update_form_validation` twice, June 2026.) Only reference components present on BOTH desktop+mobile `/startup` (demographics fields + `user-info-store`). Consent is recorded before Start and read from `user-info-store`: desktop via `handle_landing_continue`, mobile via `record_mobile_consent` (UA-guarded so it doesn't race the desktop landing). Both set `consent_completed=True`; the `display_page` guard gates `/prediction` (both) and desktop `/startup` on it; mobile `/startup` is exempt. **Any new synthetic user (chart mode, staging node, test) must set `consent_completed=True`.**
