...
- **localStorage is device-local — no cross-device resume.** A game started on a phone cannot continue on a desktop; only the read-only on-disk share record crosses devices. Architecture, not a bug.
